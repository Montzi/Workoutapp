...
Version 5. **One file plus an images folder.** Everything is inside `index.html`, so an update means replacing a single file.

## The repo

    index.html    the whole app
    images/       one PNG per exercise, see IMAGE-FILENAMES.md
    Code.gs       the Google Sheet backend, paste into Apps Script, NOT part of the website
    README.md     this

## The exercise pictures

Make a folder called `images` at the repo root and put your PNGs in it. The app builds each
filename from the exercise, so nothing needs wiring up by hand:

    Group_Exercise_Name.png

Your first one, `Chest_Flat_Bench_Press.png`, already matches. `IMAGE-FILENAMES.md` lists all 44
names exactly as the app will ask for them. Spelling and capitalisation are case sensitive on
GitHub Pages.

A missing image is not an error. That exercise quietly falls back to the animated figure, so you
can add pictures a few at a time and the app keeps working throughout.
