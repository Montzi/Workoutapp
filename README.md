...
Version 19. **One HTML file, plus your pictures as files.** Everything is inside `index.html`, so an update means replacing a single file.
...
## Update Code.gs for this version

The backend changed. Paste the new `Code.gs` over the old one and redeploy, or the second device
will keep showing an empty week.

It adds a **Run** column to Sets and Sessions and a new **Prefs** tab. Your existing tabs are
repaired in place: the script notices the missing columns and adds the headers itself. Nothing you
have already recorded is touched or lost. Rows written before this version have an empty Run,
which the app reads as one older run and handles fine.
